...
Run `tools/recox_endpoint_recon.py` to discover historical endpoints from Wayback Machine, Common Crawl, OTX, and URLScan:

```bash
# Full recon for a domain
python3 tools/recox_endpoint_recon.py target.com -o outputs/<program>/recon/endpoints.txt

# Interesting files only (js, json, xml, php, config, env, etc.)
python3 tools/recox_endpoint_recon.py target.com -i -o outputs/<program>/recon/endpoints_interesting.txt

# Specific sources only
python3 tools/recox_endpoint_recon.py target.com --sources wayback,otx
```
